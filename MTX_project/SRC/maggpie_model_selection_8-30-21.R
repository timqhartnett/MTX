...
#63.790354 1611.136648  125.190184    1.822308    8.124344   14.013793    0.481366
#0.5741244 42.0824410  1.1724460  0.0200736  0.7365421  0.2940796  0.1022612
...
model = models[[10]]
save(model,file = paste(MODEL_DIR,'10fold.magpie.8-30-21.rda',sep = '/'))
#training
train = kfold.split[[10]]
test = kfold.split[[20]]
write.csv(train,paste(DATA_DIR,'10foldcv_train_8-30-21.csv',sep = '/'))
write.csv(train,paste(DATA_DIR,'10foldcv_test_8-30-21.csv',sep = '/'))
### model performance ####
load(paste(MODEL_DIR,'10fold.magpie.8-30-21.rda',sep = '/'))
svr.train.predictions = fn.ensemble.predict(train,model = model)
svr.train.performance = cbind(train$response,svr.train.predictions)
...
Rsqrd = cor(train$response,svr.train.predictions$prediction.mean)^2

svr.test.predictions = fn.ensemble.predict(test,model = model)
svr.test.performance = cbind(test$response,svr.test.predictions)
...
Rsqrd.test = cor(svr.test.predictions$prediction.mean, test$response)
