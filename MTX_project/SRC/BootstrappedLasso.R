...
library(glmnet)
library(matrixStats)
theating.model.lasso = fn.boot.lasso(theating.train,42,1000,'8-10-21.lasso.10features.rda',model_directory = MODEL_DIR)

#performance
load(paste(MODEL_DIR,'8-10-21.lasso.10features.rda',sep = '/'))

lasso.train.predictions <- fn.predict.lasso(theating.model.lasso,theating.train)[,10:12]
...
lasso.test.predictions <- fn.predict.lasso(theating.model.lasso,theating.test)[,10:12]
...
write.csv(lasso.performance, file = paste(ANALYSIS_DIR,'best_comp_model_lasso_8-10-21.csv',sep = '/'))
